...
export const executeLogin = async (email: string, password: string, locale: string = languageDefault!) => {
    ...
    const findUser: UserProject = await userModel.findOne({ email: email }).populate('roles').populate('organizations')
    if (!findUser) {
    ...
    if (role.name === 'SuperAdmin') {
        ...
        if (findUser.organizations && findUser.organizations[0]) {
            const userOrgId: any = findUser.organizations[0]
            const isPasswordMatching: boolean = await bcrypt.compare(password, findUser.password)
            ...
            throw new HttpException(410, __({ phrase: 'organization not user', locale }, { email: email }))
        }
    }
}
